...
impl<'a> Iom<'_> {
    ...
    fn write_data(&self) {
        ...
        self.buffer.map(|buf| {
            ...
            if data_pushed == self.write_index.get() {
    ...
    fn read_data(&self) {
        ...
        self.buffer.map(|buf| {
            ...
            if regs.fifoptr.read(FIFOPTR::FIFO1SIZ) > (len % 4) as u32 {
                // Check if we have any left over data
                if len % 4 == 1 {
                    let d = regs.fifopop.get().to_ne_bytes();
